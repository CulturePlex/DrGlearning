aplic.controllers.DateTimePicker = Ext.extend(Ext.Picker, {
            /**
     * @cfg {Number} yearFrom
    ...
    initComponent:function() {
        ...
        for (i = 0, ln = Date.monthNames.length; i < ln; i++) {
        	var temp = "error";
        	if(Date.monthNames[i]=="January")
        	{
        		temp="Enero";
        	}
        	if(Date.monthNames[i]=="February")
        	{
        		temp="Febrero";
        	} 
        	if(Date.monthNames[i]=="March")
        	{
        		temp="Marzo";
        	} 
        	if(Date.monthNames[i]=="April")
        	{
        		temp="Abril";
        	}
        	if(Date.monthNames[i]=="May")
        	{
        		temp="Mayo";
        	}  
        	if(Date.monthNames[i]=="June")
        	{
        		temp="Junio";
        	} 
        	if(Date.monthNames[i]=="July")
        	{
        		temp="Julio";
        	} 
        	if(Date.monthNames[i]=="August")
        	{
        		temp="Agosto";
        	} 
        	if(Date.monthNames[i]=="September")
        	{
        		temp="Septiembre";
        	} 
        	if(Date.monthNames[i]=="October")
        	{
        		temp="Octubre";
        	} 
        	if(Date.monthNames[i]=="November")
        	{
        		temp="Noviembre";
        	} 
        	if(Date.monthNames[i]=="December")
        	{
        		temp="Diciembre";
        	}  
            months.push({
        ...
        */
        ...
        aplic.controllers.DateTimePicker.superclass.initComponent.call(this);
    },
    ...
    horaActual : function(values, animated) {
      	var ahora=new Date();
      	
		this.value.day= ahora.getDate();
		this.value.month= ahora.getMonth()+1;
		this.value.year= ahora.getFullYear();
		this.value.hour=ahora.getHours();
		this.value.minute= ahora.getMinutes();
    ...
    setFecha : function(fecha) {
		...
		this.value.day= fecha.getDate();
		this.value.month= fecha.getMonth()+1;
		this.value.year= fecha.getFullYear();
		this.value.hour=fecha.getHours();
		this.value.minute= fecha.getMinutes();
		
		this.items.items[0].selectedIndex= 2;
		
	
    } 
});
 
Ext.reg('datetimepicker', aplic.controllers.DateTimePicker);
