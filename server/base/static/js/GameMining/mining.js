...
var GraphEditor = {
  ...
  addEdge: function(_source, _type, _target, _properties){
    ...
    if (!this.nodeExists(edgeSource)){
      alert("ERROR adding Source to an Edge: Unknown node: " + edgeSource);
      return;
    }
    if (!this.nodeExists(edgeTarget)){
      alert("ERROR adding Target to an Edge: Unknown node: " + edgeTarget);
      return;
    }
    if (edgeType == "") {
          ...
          reader.onload = (function(theFile) {
            return function(e) {
              ...
              $(gexfContent).find('edge').each(function(){
                ...
                var targetId = $(this).attr('target');
                var source;
                //Old way to find the source, improved to get complex string ids
                //var source = $(gexfContent).find('node#'+sourceId).attr('label');
                $(gexfContent).find('node').each(function(){
                    if ($(this).attr('id') === sourceId)
                    {
                        source = $(this).attr('label');
                    }
                });
                var target;
                //Old way to find the target, improved to get complex string ids
                //var target = $(gexfContent).find('node#'+targetId).attr('label');
                $(gexfContent).find('node').each(function(){
                    if ($(this).attr('id') === targetId)
                    {
                        target = $(this).attr('label');
                    }
                });
                var type = $(this).attr('label');
  ...
  init: function(){
    ...
    $('#extract-fb').click(function(){
        jQuery.get( "https://www.googleapis.com/freebase/v1/mqlread","query={%22name%22:%22Mogwai%22,%22type%22:%22/music/artist%22,%22*%22:null}" );
    });
