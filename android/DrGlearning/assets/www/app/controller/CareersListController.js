...
Ext
		.define(
				'DrGlearning.controller.CareersListController',
				{
					...
					initializate : function() {
						...
						this.careerController = this.getApplication()
								.getController('CareerController');
						this.levelController = this.getApplication()
								.getController('LevelController');
						this.daoController = this.getApplication()
								.getController('DaoController');
						console.log(this);
						...
						this.control({
							...
							/*
							 * 'selectfield[name=knnowledge_field]': { change:
					...
					addOrStartCareer : function(list, itemIndex, item, career) {
						...
						if (career.data.installed == "false") {
							Ext.Msg
									.confirm(
											"Install Career?",
											"Are you sure you want to install this career?",
											function(answer, pako) {
					...
					filterCareers : function() {
						...
						store.filter("installed", "true");
						var view1 = this.getCareersframe();
						var careerStateSelected = Ext.ComponentQuery
								.query('selectfield[name=state]')[0];
					...
					/*
					 * filterCareersByKnowledge: function(){ var store =
					 * this.getCareersStore(); store.clearFilter();
					 * store.filter("installed", "false"); var view1 =
					 * this.getCareersframe(); store.filter("started", "false");
					 * if
					 * (view1.down('selectfield[name=knnowledge_field]').getValue() ==
					 * 'notYet') { store.filter("started", "false"); } if
					 * (view1.down('selectfield[name=state]').getValue() ==
					 * 'inProgress') { store.filter("started", "true"); }
					 * store.load(); view1.down('careerslist').refresh(); },
					 */
					/*
					...
					addCareer : function() {
						...
						knowledgeFields = this.daoController
								.getknowledgesFields();
						...
						view12.down('selectfield[name=knnowledge_field]')
								.setOptions(options);
						var store = Ext.getStore('Careers');
						store.clearFilter();
						...
						view12.show();
						view12.down('careerslist').refresh();
						console.log(view12.down('careerslist').getStore()
								.getCount());
					},
					/*
					...
					search : function(values, form) {
						...
						filters.push(new Ext.util.Filter({
							filterFn : function(item) {
								return item.data.name.toLowerCase().indexOf(
										form) != -1
										|| item.data.description.toLowerCase()
												.indexOf(form) != -1;
							}
						}));
						store.clearFilter();
