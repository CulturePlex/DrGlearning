    ...
    Ext.apply(this, {
        ...
        DATE: {
            convert: function(v) {
                ...
                if (!v) {
                	console.log('v es null');
                    return null;
                }
                if (Ext.isDate(v)) {
                	console.log('Esto es ya una fecha');
                    return v;
                }
                if (df) {
                    if (df == 'timestamp') {
                    	console.log('v es un timestamp');
                        return new Date(v*1000);
                    }
                    if (df == 'time') {
                    	console.log('v es un time');
                        return new Date(parseInt(v, 10));
                    }
                    console.log ('Parseando :  ' + v + ' con ' + df + ' ' + Date.parseDate(v , df));
                    
                    return Date.parseDate(v, df);
                }
                
                var parsed = Date.parse(v);
                console.log ('Parseado : ' + parsed + ' ' + v);
 ...
 (function() {
    ...
    Ext.apply(Date, {
        ...
        parseDate: function(input, format, strict) {
        	console.log('Tratando de parsear ' + input + " con formato " + format)
...
Ext.DataView = Ext.extend(Ext.Component, {
    ...
    initComponent : function(){
        ...
        if (!isDef(this.tpl) || !isDef(this.store) || !isDef(this.itemSelector)) {
        	console.log(this.tpl+' '+this.store+' '+this.itemSelector);
